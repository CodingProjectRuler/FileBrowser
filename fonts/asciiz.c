...
static int ASCIIFontInit(char *pcFontFile, unsigned int dwFontSize)
{
	...
	if(dwFontSize != 16){
		printf("Can't support %d font size!\n", dwFontSize);
		return -1;
	}
	return 0;
}
							    /*_*/
/*iX,iYÊÇÈËÎªÆÚÍûµÄÎ»ÖÃ|
    iLeft, iTop ÊÇÊµ¼Ê¿ªÊ¼µÄÎ»ÖÃ,ÔÚfreetypeÀïÃæÓÐÐ©ÐíÇø±ð
  */
static int ASCIIGetFontBitmap(unsigned int dwCode, PT_FontBitMap ptFontBitMap)
{
	if((dwCode > (unsigned int)0x80) || (g_dwFontSize  != 16)){
//		printf("Is not ASCII code!\n");
		return -1;
	}
	/*¸ù¾ÝÂëÖµµÃµ½×ÖÌåÊý¾Ý²¢ÉèÖÃÎ»Í¼µÄÏÔÊ¾ÊôÐÔ*/
	ptFontBitMap->iLeft = ptFontBitMap->iX;
	ptFontBitMap->iTop = ptFontBitMap->iY;
	ptFontBitMap->iXmax = ptFontBitMap->iX + 8;
	ptFontBitMap->iYmax = ptFontBitMap->iY + 16;
	ptFontBitMap->iNextX= ptFontBitMap->iX + 8;
